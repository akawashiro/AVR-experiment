...
#define IF_BUS 8                /* Data bus width (4 or 8) */
#define DELAY_US(n) delay_us(n) /* Delay n microseconds */
#define IF_INIT() \
    {} /* Initialize control port */
#define IF_DLY60() \
    {} /* Delay >=60ns (can be blanked for most uC) */
#define IF_DLY450() \
    {}                     /* Delay >=450ns@3V, >=250ns@5V */
#define E1_HIGH() bset(P0) /* Set E(E1) high */
#define E1_LOW() bset(P0)  /* Set E(E1) low */
#define E2_HIGH() bclr(P1) /* Set E2 high (dual controller only) */
#define E2_LOW() bclr(P1)  /* Set E2 low (dual controller only) */
#define RS_HIGH() bset(P2) /* Set RS high */
#define RS_LOW() bclr(P2)  /* Set RS low */
#define OUT_DATA(d)                                                        \
    outp(Q, d); /* Output a byte d on the bus (higher 4 bits of d in 4-bit \
                   mode) */
#endif
...
void lcd_write(
    uint8_t reg, /* b0:command(0)/data(1), b2-1:E1(2)/E2(1)/both(0)(don't care
                    on single controller), b3:write high nibble only(don't care
                    on 8-bit I/F) */
...
void lcd_putc(uint8_t chr) {
    ...
    if (LCD_IF_SPLIT && Column == _LCD_COLS / 2) lcd_write(0, 0x40);

    if (Column >= _LCD_COLS) lcd_locate(Row + 1, 0);
}
...
#if _USE_BAR
void lcd_put_bar(
    uint16_t val,  /* Bar length (0 to _MAX_BAR represents bar length from left
                      end) */
    uint8_t width, /* Display area (number of chars from cursor position) */
    uint8_t chr    /* User character code (2 chars used from this) */
) {
    static const uint8_t ptn[] = {0xE0, 0xE0, 0xE0, 0xC0, 0xC0, 0xC0, 0x80, 0,
...
#if _USE_POINT
void lcd_put_point(
    uint16_t val,  /* Dot position (0 to _MAX_POINT represents left end to write
                      end) */
    uint8_t width, /* Display area (number of chars from cursor position) */
    uint8_t chr    /* User character code (2 chars used from this) */
) {
    static const uint8_t ptn[] = {0x06, 0x0C, 0x0C, 0x0C, 0x18, 0x18, 0x18, 0,
